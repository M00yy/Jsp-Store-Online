...
import com.gm.service.impl.ManagerServiceImpl;
...
//			getServletContext().setAttribute("menus", menus);//放到应用域中
	...
	private void sendBook(HttpServletRequest request,
		...
		response.sendRedirect(request.getContextPath()+"/manager/ManagerServlet?operation=showPayedOrders");
	}
	//查看已付款的订单
	...
	private void delBook(HttpServletRequest request,
		...
		s.delBook(bookId);
		request.setAttribute("message", "<script type='text/javascript'>alert('已删除')</script>");
		request.getRequestDispatcher("/manager/ManagerServlet?operation=showBooks").forward(request, response);
	}
